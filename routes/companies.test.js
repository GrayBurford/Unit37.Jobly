...
describe("GET /companies", function () {
  ...
  test("fails: invalid query string key", async function () {
    const response = await request(app)
      .get("/companies")
      .query({
        "FAILNAME" : "FAIL",
    ...
    expect(response.body.error.message).toContain("invalid keys in your request");
  })

  test("fails: min > max employees in query", async function () {
    const response = await request(app)
      .get("/companies")
      .query({
        "minEmployees" : "30",
    ...
    expect(response.statusCode).toBe(400);
    expect(response.body.error.message).toContain("maxEmployees must be greater than minEmployees!")
  })
